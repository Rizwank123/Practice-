...
def scrape_additional_info(url):
    ...
    return product_description, manufacturer
...
for index, item in enumerate(data):
    url = item[0]
    product_description, manufacturer = scrape_additional_info(url)

    # Update the data with additional information
    data[index].extend([product_description, manufacturer])

# Export the updated data to a CSV file
csv_file = 'product_data.csv'
header = ['Product URL', 'Product Name', 'Product Price', 'Rating', 'Number of Reviews', 'Product Description', 'Manufacturer']
